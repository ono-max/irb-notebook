...
export function activate(context: vscode.ExtensionContext) {
  const cellToFileMap = new Map<string, string>();
  const fileToCellMap = new Map<string, vscode.NotebookCell>();
  context.subscriptions.push(
    vscode.workspace.registerNotebookSerializer('irb-notebook', new RdbgNotebookSerializer()),

    new RdbgNotebookController(),

    vscode.commands.registerCommand('rdbg.debugCell', (cell: vscode.NotebookCell) => {
      debugCell(cell, cellToFileMap, fileToCellMap);
    }),

    vscode.debug.registerDebugAdapterTrackerFactory('rdbg', new RdbgDebugAdapterTrackerFactory(cellToFileMap, fileToCellMap))
  );
}
...
class RdbgNotebookSerializer implements vscode.NotebookSerializer {
  ...
  return new Promise((resolve, reject) => {
    const clientReq = http.request(opts, (res) => {
      if (res.statusCode !== 200) {
        console.error('not expected code')
      }
...
async function debugCell(cell: vscode.NotebookCell, cellToFileMap: Map<string, string>, fileToCellMap: Map<string, vscode.NotebookCell>) {
  const path = await createFile();
  await debug(path);

  async function createFile() {
    const text = `require_relative "/Users/s15236/workspace/debug/lib/debug/session";DEBUGGER__.open_tcp(port: 12345)\n` + cell.document.getText();
    const body = { text };
    const res = await postJSON(body, '/files');
    cellToFileMap.set(cell.document.uri.toString(), res.path);
    fileToCellMap.set(res.path, cell);
    fileToCellMap.set(`/private${res.path}`, cell);
    return res.path;
  }
